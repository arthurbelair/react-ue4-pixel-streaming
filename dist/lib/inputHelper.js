...
function inputHelper(webRtcPlayerObj, settings) {
  var _emitter = (0, _emitter2.default)(webRtcPlayerObj, settings),
      emitMouseDown = _emitter.emitMouseDown,
      emitMouseUp = _emitter.emitMouseUp,
      emitMouseWheel = _emitter.emitMouseWheel,
      emitMouseMove = _emitter.emitMouseMove,
      sendInputData = _emitter.sendInputData; // https://developer.mozilla.org/en-US/docs/Web/API/MouseEvent/button
  ...
  function registerTouchEvents(playerElement, settings) {
    ...
    if (settings.inputOptions.fakeMouseWithTouches) {
      ...
      playerElement.ontouchstart = function (e) {
        ...
        emitTouchData(_types.MessageType.TouchStart, e.changedTouches);
        e.preventDefault();
      };
      ...
      playerElement.ontouchend = function (e) {
        ...
        emitTouchData(_types.MessageType.TouchEnd, e.changedTouches); // Re-cycle unique identifiers previously assigned to each touch.
      ...
      playerElement.ontouchmove = function (e) {
        ...
        emitTouchData(_types.MessageType.TouchMove, e.touches);
        e.preventDefault();
      };
    }
  }
  ...
  function registerMouseEnterAndLeaveEvents(playerElement, settings) {
    playerElement.onmouseenter = function (e) {
      if (settings.print_inputs) {// console.log("mouse enter");
      }

      var Data = new DataView(new ArrayBuffer(1));
      Data.setUint8(0, _types.MessageType.MouseEnter);
      sendInputData(Data.buffer);
      playerElement.pressMouseButtons(e);
    };

    playerElement.onmouseleave = function (e) {
      if (settings.print_inputs) {// console.log("mouse leave");
      }

      var Data = new DataView(new ArrayBuffer(1));
      Data.setUint8(0, _types.MessageType.MouseLeave);
      sendInputData(Data.buffer);
  ...
  function registerInputs(playerElement, settings) {
    //TODO: キーボード設定
    registerKeyboardEvents(settings); //TODO: マウス設定

    switch (settings.inputOptions.controlScheme) {
      case _types.ControlSchemeType.HoveringMouse:
        registerHoveringMouseEvents(playerElement);
        break;

      case _types.ControlSchemeType.LockedMouse:
        registerLockedMouseEvents(playerElement);
        break;

      default:
        console.log("ERROR: Unknown control scheme ".concat(settings.inputOptions.controlScheme));
        registerLockedMouseEvents(playerElement);
        break;
    }
  ...
  function registerKeyboardEvents(settings) {
    document.onkeydown = function (e) {
      ...
      sendInputData(new Uint8Array([_types.MessageType.KeyDown, e.keyCode, e.repeat]).buffer);

      if (settings.inputOptions.suppressBrowserKeys // &&
    ...
    document.onkeyup = function (e) {
      ...
      sendInputData(new Uint8Array([_types.MessageType.KeyUp, e.keyCode]).buffer);

      if (settings.inputOptions.suppressBrowserKeys && isKeyCodeBrowserKey(e.keyCode)) {
        e.preventDefault();
      }
    };
    ...
    document.onkeypress = function (e) {
      ...
      data.setUint8(0, _types.MessageType.KeyPress);
