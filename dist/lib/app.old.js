...
var webRtcPlayer = require('./webRtcPlayer');
...
var matchViewportResolution; // TODO: Remove this - workaround because of bug causing UE to crash when switching resolutions too quickly
...
function showPlayOverlay() {
  ...
  setOverlay('clickableState', img, function (event) {
...
function setupWebRtcPlayer(htmlElement, clientConfig) {
  ...
  socket.on('webrtc-answer', function (webRTCData) {
    ...
    var printInterval = 5 * 60 * 1000;
    /*Print every 5 minutes*/

    var nextPrintDuration = printInterval;
    ...
    webRtcPlayerObj.onAggregatedStats = function (aggregatedStats) {
      ...
      var timeFormat = new Intl.NumberFormat(window.navigator.language, {
        maximumFractionDigits: 0,
        minimumIntegerDigits: 2
      });
      var statsText = ''; // Calculate duration of run

      var runTime = (aggregatedStats.timestamp - aggregatedStats.timestampStart) / 1000;
      var timeValues = [];
      var timeDurations = [60, 60];

      for (var timeIndex = 0; timeIndex < timeDurations.length; timeIndex++) {
        timeValues.push(runTime % timeDurations[timeIndex]);
        runTime = runTime / timeDurations[timeIndex];
      }

      timeValues.push(runTime);
      var runTimeSeconds = timeValues[0];
      var runTimeMinutes = Math.floor(timeValues[1]);
      var runTimeHours = Math.floor([timeValues[2]]);
      var receivedBytesMeasurement = 'B';
      var receivedBytes = aggregatedStats.hasOwnProperty('bytesReceived') ? aggregatedStats.bytesReceived : 0;
      var dataMeasurements = ['kB', 'MB', 'GB'];

      for (var index = 0; index < dataMeasurements.length; index++) {
      ...
      ;
      statsText += "Duration: ".concat(timeFormat.format(runTimeHours), ":").concat(timeFormat.format(runTimeMinutes), ":").concat(timeFormat.format(runTimeSeconds), "</br>");
      statsText += "Video Resolution: ".concat(aggregatedStats.hasOwnProperty('frameWidth') && aggregatedStats.frameWidth && aggregatedStats.hasOwnProperty('frameHeight') && aggregatedStats.frameHeight ? aggregatedStats.frameWidth + 'x' + aggregatedStats.frameHeight : 'N/A', "</br>");
      statsText += "Received (".concat(receivedBytesMeasurement, "): ").concat(numberFormat.format(receivedBytes), "</br>");
      statsText += "Frames Decoded: ".concat(aggregatedStats.hasOwnProperty('framesDecoded') ? numberFormat.format(aggregatedStats.framesDecoded) : 'N/A', "</br>");
      statsText += "Packets Lost: ".concat(aggregatedStats.hasOwnProperty('packetsLost') ? numberFormat.format(aggregatedStats.packetsLost) : 'N/A', "</br>");
      statsText += "Bitrate (kbps): ".concat(aggregatedStats.hasOwnProperty('bitrate') ? numberFormat.format(aggregatedStats.bitrate) : 'N/A', "</br>");
      statsText += "Framerate: ".concat(aggregatedStats.hasOwnProperty('framerate') ? numberFormat.format(aggregatedStats.framerate) : 'N/A', "</br>");
      statsText += "Frames dropped: ".concat(aggregatedStats.hasOwnProperty('framesDropped') ? numberFormat.format(aggregatedStats.framesDropped) : 'N/A', "</br>");
      statsText += "Latency (ms): ".concat(aggregatedStats.hasOwnProperty('currentRoundTripTime') ? numberFormat.format(aggregatedStats.currentRoundTripTime * 1000) : 'N/A', "</br>");
      ...
      if (print_stats) {
        if (aggregatedStats.timestampStart) {
          if (aggregatedStats.timestamp - aggregatedStats.timestampStart > nextPrintDuration) {
            ...
            if (socket.connected) socket.emit('webrtc-stats', aggregatedStats);
...
function resizePlayerStyleToFillWindow(playerElement) {
  var videoElement = playerElement.getElementsByTagName("VIDEO"); // Fill the player display in window, keeping picture's aspect ratio.
  ...
  if (isNaN(videoAspectRatio)) {
    ...
    styleHeight = window.innerHeight;
    styleTop = 0;
    styleLeft = Math.floor((window.innerWidth - styleWidth) * 0.5); //Video is now 100% of the playerElement, so set the playerElement style
    ...
    styleLeft = 0; //Video is now 100% of the playerElement, so set the playerElement style
...
function resizePlayerStyle(event) {
  ...
  updateVideoStreamSize(); // Calculating and normalizing positions depends on the width and height of
  // the player.
...
function emitDescriptor(messageType, descriptor) {
  ...
  var descriptorAsString = JSON.stringify(descriptor); // Add the UTF-16 JSON string to the array byte buffer, going two bytes at
  // a time.
...
function setupNormalizeAndQuantize() {
  ...
  if (playerElement && videoElement.length > 0) {
    ...
    var videoAspectRatio = videoElement[0].videoHeight / videoElement[0].videoWidth; // Unsigned XY positions are the ratio (0.0..1.0) along a viewport axis,
    ...
    if (playerAspectRatio > videoAspectRatio) {
      ...
      var ratio = playerAspectRatio / videoAspectRatio; // Unsigned.
...
function emitMouseMove(x, y, deltaX, deltaY) {
  if (print_inputs) {
    console.log("x: ".concat(x, ", y:").concat(y, ", dX: ").concat(deltaX, ", dY: ").concat(deltaY));
  }
...
function emitMouseDown(button, x, y) {
  if (print_inputs) {
    console.log("mouse button ".concat(button, " down at (").concat(x, ", ").concat(y, ")"));
  }
...
function emitMouseUp(button, x, y) {
  if (print_inputs) {
    console.log("mouse button ".concat(button, " up at (").concat(x, ", ").concat(y, ")"));
  }
...
function emitMouseWheel(delta, x, y) {
  if (print_inputs) {
    console.log("mouse wheel with delta ".concat(delta, " at (").concat(x, ", ").concat(y, ")"));
  }
...
function registerTouchEvents(playerElement) {
  ...
  function emitTouchData(type, touches) {
    ...
    for (var t = 0; t < touches.length; t++) {
      ...
      if (print_inputs) {
        console.log("F".concat(fingerIds[touch.identifier], "=(").concat(x, ", ").concat(y, ")"));
      }
  ...
  if (inputOptions.fakeMouseWithTouches) {
    ...
    playerElement.ontouchend = function (e) {
      for (var t = 0; t < e.changedTouches.length; t++) {
        ...
        if (touch.identifier === finger.id) {
          var x = touch.clientX - playerElementClientRect.left;
          var y = touch.clientY - playerElementClientRect.top;
          emitMouseUp(MouseButton.MainButton, x, y); // Hack: Manual mouse leave event.
    ...
    playerElement.ontouchend = function (e) {
      ...
      emitTouchData(MessageType.TouchEnd, e.changedTouches); // Re-cycle unique identifiers previously assigned to each touch.
...
function registerKeyboardEvents() {
  document.onkeydown = function (e) {
    if (print_inputs) {
      console.log("key down ".concat(e.keyCode, ", repeat = ").concat(e.repeat));
    }
  ...
  document.onkeyup = function (e) {
    if (print_inputs) {
      console.log("key up ".concat(e.keyCode));
    }
  ...
  document.onkeypress = function (e) {
    if (print_inputs) {
      console.log("key press ".concat(e.charCode));
    }
...
function onExpandOverlay_Click() {
  var subElement = document.getElementById('overlaySettings');
