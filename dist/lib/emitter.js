...
function emitter(webRtcPlayerObj, settings) {
  var normalizeAndQuantizeUnsigned = function (webRtcPlayerObj) {
    var playerElement = webRtcPlayerObj.video;
    var videoElement = webRtcPlayerObj.video;
    return function (x, y) {
  ...
  var normalizeAndQuantizeSigned = function (webRtcPlayerObj) {
    var playerElement = webRtcPlayerObj.video;
    var videoElement = webRtcPlayerObj.video;
    return function (x, y) {
      var playerAspectRatio = playerElement.clientHeight / playerElement.clientWidth;
  ...
  }(webRtcPlayerObj); // A generic message has a type and a descriptor.
  ...
  function emitUIInteraction(descriptor) {
    emitDescriptor(_types.MessageType.UIInteraction, descriptor);
  ...
  function emitCommand(descriptor) {
    emitDescriptor(_types.MessageType.Command, descriptor);
  }
  ...
  function emitMouseMove(x, y, deltaX, deltaY) {
    ...
    var coord = normalizeAndQuantizeUnsigned(x, y); //let coord = {x,y};

    var delta = normalizeAndQuantizeSigned(deltaX, deltaY); //let delta = {deltaX, deltaY};

    var Data = new DataView(new ArrayBuffer(9));
    Data.setUint8(0, _types.MessageType.MouseMove);
  ...
  function emitMouseDown(button, x, y) {
    ...
    Data.setUint8(0, _types.MessageType.MouseDown);
  ...
  function emitMouseUp(button, x, y) {
    ...
    var Data = new DataView(new ArrayBuffer(6));
    Data.setUint8(0, _types.MessageType.MouseUp);
    Data.setUint8(1, button);
    Data.setUint16(2, coord.x, true);
    Data.setUint16(4, coord.y, true);
    sendInputData(Data.buffer);
  ...
  function emitMouseWheel(delta, x, y) {
    ...
    Data.setUint8(0, _types.MessageType.MouseWheel);
